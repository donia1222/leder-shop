...
try {
    ...
    if ($pdo->query("SELECT COUNT(*) FROM shipping_zones")->fetchColumn() == 0) {
        $pdo->exec("INSERT INTO shipping_zones (name, countries, enabled) VALUES
            ...
            ('Europa', 'DE,FR,IT,AT,ES,NL,BE,PL,PT,CZ,DK,SE,FI,NO,HU,RO,HR,SK,SI,LU', 1),
            ('International', '*', 1)");
    }

    // Migration: update Schweiz zone to include Liechtenstein (keep enabled)
    ...
    // Remove LI from Europa zone if it was there
    try { $pdo->exec("UPDATE shipping_zones SET countries = REPLACE(REPLACE(countries, ',LI', ''), 'LI,', '') WHERE name LIKE 'Europa%'"); } catch (Exception $e) {}

    // Migration: replace old weight ranges with 4 PostPac categories
